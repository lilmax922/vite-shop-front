    ...
    if (error.response.status === 401) {
      ...
      if (!['/users/extend', '/users/login', '/users/logout'].includes(error.config.url)) {
        ...
        return apiAuth.patch('/users/extend').then(({ data }) => {
          user.token = data.result // 更新 user store 的 JWT
          error.config.headers.authorization = 'Bearer ' + user.token // 修改原請求的 JWT
          return axios(error.config) // 重新傳送原請求
        }).catch(_ => {
          user.logout() // 重新登入失敗，強制登出
          return Promise.reject(error) // 回傳原本的錯誤到呼叫的地方
        })
      }
    }
  }
  return Promise.reject(error) // 如果失敗的請求沒回應，回傳原本的錯誤到呼叫的地方
})
